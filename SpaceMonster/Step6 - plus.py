# 우주 괴물이 화면의 위에서 아래로 내려옴. 속도를 무작위로 5~9가 되도록함.
# 우주괴물을 맞힐때마다 우주선의 그림이 다른 우수선으로 변경
# 우주 괴물이 살아서 바닥까지 도달하면 점수에서 -1, 마이너스가 될수도 있음
# 우주선과 괴물이 부딪히면 '우주선 폭파' 메시지를 화면에 출력, 5초간 화면이 멈추고 프로그램을 종료한다.
